...
df <- map(
  seq(10, 200, by = 10),
  \(x) {
    bench::mark(classic = fib_classic(x), 
            phi = fib_phi(x)) |> 
      mutate(n = x)
  }) |> 
  list_rbind()
df |> 
  ...
  geom_point() +
  labs(y = expression("Median time to compute " * F[n] * "  (seconds)"), x = "n",
       title = expression("Comparison of methods up to "*F[200])) + 
  ggthemes::theme_few()
```

Sure enough, you can see that the `phi` method overtakes the `classic` method somewhere around $F_200$. More importantly, there's a clear difference in the _shape_ of these two curves: the `classic` method is a very obvious straight line, whereas the `phi` method is logarithmic.

There is a problem with what I've just done. The Fibonacci numbers grow really fast, and in fact both of these methods are producing numbers larger than R knows how to think about. They already disagree on the value of $F_{100}$, and in fact neither one finds the correct value of $F_{100} = 354224848179261915075$.
