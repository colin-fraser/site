---
title: Tracking the COVID-19 outbreak in Canada
author: Colin Fraser
date: '2020-03-27'
slug: tracking-covid-19-in-canada
categories: []
tags: []
aliases:
  - /covid19/
  - /covid-19/
  - /covid/
description: ''
---

```{r include = FALSE}
library(dplyr)
library(readr)
library(ggplot2)
library(fs)
library(purrr)
library(stringr)
library(lubridate)
library(tidyr)
library(forcats)

project_path <- path_expand("~/projects")

theme_set(ggthemes::theme_few() +
    theme(legend.position = "none", panel.grid = element_line("grey95")))

load_all_raw_data2 <- function() {
  data_path <- fs::path(project_path, 'COVID-19/csse_covid_19_data/csse_covid_19_daily_reports')
  files <- dir_ls(data_path)
  chunk_1 <- files[1:11] %>%
    map_df(read_csv, col_types = 'cccnnn', skip = 1,
        col_names = c('province_state', 'country_region', 'last_update', 'confirmed', 'deaths', 'recovered'),
        .id = 'date') %>%
    mutate(date = str_extract(date, '\\d+-\\d+-\\d+')) %>%
    mutate(date = mdy(date), last_update = lubridate::mdy_hm(last_update))

  chunk_2 <- files[12:39] %>%
    map_df(read_csv, skip = 1, col_names= c('province_state', 'country_region', 'last_update', 'confirmed', 'deaths', 'recovered'),
           col_types = 'ccTnnn',
           .id = 'date') %>%
    mutate(date = str_extract(date, '\\d+-\\d+-\\d+')) %>%
    mutate(date = mdy(date))

  chunk_3 <- files[40:60] %>%
    map_df(read_csv, skip = 1, c('province_state', 'country_region', 'last_update', 'confirmed', 'deaths', 'recovered', 'lat', 'lon'),
           .id = 'date', col_types = 'ccTnnnnn') %>%
    mutate(date = str_extract(date, '\\d+-\\d+-\\d+')) %>%
    mutate(date = mdy(date))

  chunk_4 <- files[61] %>%
    map_df(read_csv, skip = 1, c('fips', 'admin2', 'province_state', 'country_region', 'last_update', 'lat', 'lon', 'confirmed', 'deaths', 'recovered',
                                 'active', 'combined_key'),
           .id = 'date', col_types = 'cccccnnnnnnc') %>%
    mutate(last_update = mdy_hm(last_update)) %>% 
    mutate(date = str_extract(date, '\\d+-\\d+-\\d+')) %>%
    mutate(date = mdy(date))
  
  chunk_5 <- files[62:(length(files)-1)] %>%
    map_df(read_csv, skip = 1, c('fips', 'admin2', 'province_state', 'country_region', 'last_update', 'lat', 'lon', 'confirmed', 'deaths', 'recovered',
                                 'active', 'combined_key'),
           .id = 'date', col_types = 'ccccTnnnnnnc') %>%
    mutate(date = str_extract(date, '\\d+-\\d+-\\d+')) %>%
    mutate(date = mdy(date))

  bind_rows(chunk_1, chunk_2, chunk_3, chunk_4, chunk_5)
}

canada_provinces <- load_all_raw_data2() %>%
    # filter(country_region == "Canada") %>%
    mutate(
      province =
        case_when(
          str_detect(province_state, "British Columbia") ~ "British Columbia",
          str_detect(province_state, "Ontario|ON$") ~ "Ontario",
          str_detect(province_state, "Quebec|QC$") ~ "Quebec",
          str_detect(province_state, "Alberta") ~ "Alberta",
          str_detect(province_state, "Manitoba") ~ "Manitoba",
          str_detect(province_state, "New Brunswick") ~ "New Brunswick",
          str_detect(province_state, "Newfoundland") ~ "Newfoundland",
          str_detect(province_state, "Northwest") ~ "Northwest Territories",
          str_detect(province_state, "Nova Scotia") ~ "Nova Scotia",
          str_detect(province_state, "Saskatchewan") ~ "Saskatchewan",
          str_detect(province_state, "Yukon") ~ "Yukon",
          str_detect(province_state, "Prince Ed") ~ "Prince Edward Island",
          str_detect(province_state, 'Washington$|WA$') ~ 'Washington State',
          str_detect(province_state, 'New York$|NY$') ~ 'New York State'
        )
    ) %>%
    filter(!is.na(province))

canada_plot <- function(variable = confirmed, x = first, filter = 10) {
  
  if(substitute(variable) == 'confirmed') {
    title <- 'Confirmed Cases'
  } else if (substitute(variable) == 'deaths') {
    title <- 'Deaths'
  }
  
  if(substitute(x) == 'first') {
    x_axis <- glue::glue('Days Since {filter} {title}')
  } else if (substitute(x) == 'date') {
    x_axis <- 'Date'
  }

  plot_data <- canada_provinces %>%
    group_by(province, date) %>%
    summarise(confirmed = sum(confirmed), deaths = sum(deaths, na.rm = T)) %>%
    filter({{variable}} >= filter) %>%
    mutate(first = as.numeric(date - min(date)))

  label_data <- plot_data %>%
    filter(first == max(first))

  M <- max(plot_data$first) + 5

  p <- plot_data %>%
    mutate(
      day2 = filter * 2^(first / 2),
      day3 = filter * 2^(first / 3),
      day4 = filter * 2^(first / 4)
    ) %>%
    ggplot(aes(x = {{x}}, y = {{variable}}, color = province)) +
    geom_point() +
    geom_line() +
    scale_y_log10(labels = scales::comma) +
    ggrepel::geom_text_repel(data = label_data, aes(label = province), size = 4) +
    labs(
      x = x_axis,
      y = glue::glue("Total {title}"),
      title = glue::glue("{title} By Province and US State"),
      caption = glue::glue("Data from https://github.com/CSSEGISandData/COVID-19, updated {maxdate}", maxdate = max(plot_data$date))
    ) 
    

  if (substitute(x) == 'first') {
    p <- p + 
    geom_line(aes(y = day2, color = NULL), linetype = "dashed", color = "grey40") +
    geom_line(aes(y = day3, color = NULL), linetype = "dashed", color = "grey40") +
    geom_line(aes(y = day4, color = NULL), linetype = "dashed", color = "grey40") +
    annotate("text", x = M - 5.5, y = filter * 2^((M - 5) / 2:4 - .25), label = paste(2:4, "days"), color = "grey", hjust = 0)
  }
  
  p
}

```
_Last updated `r strftime(Sys.time(), usetz = T)`._

The Financial Times [has a nice COVID-19 dashboard](https://www.ft.com/coronavirus-latest) that shows the growth in confirmed COVID-19 cases around the world, including for US states, but I haven't seen a great version of these charts for Canadian provinces. I borrowed some of the methodology from that page to come up with some Canada-centric charts. Hopefully these will be helpful for some people. The data comes from the [Johns Hopkins COVID-19 resource center](https://coronavirus.jhu.edu/map.html), which they've shared publicly in [this GitHub repo](https://github.com/CSSEGISandData/COVID-19).

I can't guarantee the accuracy of the information here, but it should be consistent with what you can find on the Johns Hopkins dashboard. Please [send me a message on Twitter](https://twitter.com/colin_fraser) if you find anything that seems suspicious.

## Current Tally

According to the latest data, as of `r canada_provinces %>% summarise(date = max(date)) %>% pull(date)` there have been `r canada_provinces %>% filter(country_region == 'Canada', date == max(date)) %>% summarise(scales::comma(sum(confirmed), accuracy = 1))` confirmed cases reported in Canada, resulting in `r canada_provinces %>% filter(country_region == 'Canada', date == max(date)) %>% summarise(scales::comma(sum(deaths), accuracy = 1))` deaths. Breakdowns by province are below.

### Confirmed Cases

```{r echo=FALSE, warning=FALSE}
canada_provinces %>% 
  filter(country_region == 'Canada') %>% 
  group_by(province, date) %>% 
  summarise(confirmed = sum(confirmed), deaths = sum(deaths, na.rm = T)) %>%
  filter(date >= max(date) - 7) %>% 
  mutate(dc = c(NA, diff(confirmed))/lag(confirmed),
         dd = c(NA, diff(deaths))/lag(deaths),
         d1 = c(NA, diff(confirmed))) %>% 
  summarise(date = last(date),
    confirmed = last(confirmed),
    new_cases = last(d1),
    dc = mean(dc, na.rm = T)) %>% 
  mutate(confirmed = scales::comma(confirmed, accuracy = 1),
         dc = scales::percent(dc, accuracy = .1)) %>% 
  mutate(province = factor(province, levels = c("Alberta", "British Columbia", "Manitoba", "New Brunswick", 
"Newfoundland", "Northwest Territories", "Nova Scotia", "Ontario", "Prince Edward Island", 
"Quebec", "Saskatchewan", "Yukon", "Total"))) %>% 
  arrange(province) %>% 
  knitr::kable(col.names = c('Province', 'Most Recent Data', 'Confirmed Cases', 'New Cases',
                             'Average Daily Growth Rate (Last 7 days)')) 
```

### Deaths

```{r echo=FALSE, warning=FALSE}
canada_provinces %>% 
  filter(country_region == 'Canada') %>% 
  group_by(province, date) %>% 
  summarise(confirmed = sum(confirmed), deaths = sum(deaths)) %>%
  filter(date >= max(date) - 7) %>% 
  mutate(dc = c(NA, diff(confirmed))/lag(confirmed),
         dd = c(NA, diff(deaths))/lag(deaths),
         d1 = c(NA, diff(deaths))) %>% 
  summarise(date = last(date),
    deaths = last(deaths), 
    d1 = last(d1),
    dc = mean(dd, na.rm = T)) %>% 
  mutate(dc = ifelse(dc == Inf, NA, dc)) %>% 
  mutate(deaths = scales::comma(deaths, accuracy = 1),
         dc = scales::percent(dc, accuracy = .1)) %>% 
  filter(deaths > 0) %>% 
  knitr::kable(col.names = c('Province', 'Most Recent Data', 'Total Deaths', 'New Deaths',
                             'Average Daily Growth Rate (Last 7 days)'))
  
```

## Confirmed Cases

### Confirmed Cases Since The 10th Case

This chart shows the number of confirmed cases in each province, with the number of days since the 10th case on the x-axis. I've included New York State and Washington State for comparison, since those are two of the major US outbreaks. The dashed lines indicate how this chart would look if doubling the number of cases took 2, 3, or 4 days.

```{r confirmed_since_start, echo=FALSE, warning=FALSE}
canada_plot()
```

### Growth Rate in Confirmed Cases Since 10th Confirmed Case

This plot shows the daily growth rate of confirmed cases since the 10th confirmed case. We want this to go to zero.

```{r growth_rate, echo=FALSE, warning=FALSE}
plotdata <- canada_provinces %>% 
  group_by(province, date) %>% 
  summarise(confirmed = sum(confirmed)) %>% 
  mutate(diff = confirmed/lag(confirmed, fill = NA) - 1) %>% 
  mutate(diff = zoo::rollmean(diff, 3, fill = NA)) %>% 
  filter(confirmed >= 10) %>% 
  mutate(days_since_10 = as.numeric(date - min(date)))
label_data <- plotdata %>% 
  filter(!is.na(diff)) %>% 
  filter(date == max(date))

update_date <- max(plotdata$date)

provinces <- unique(plotdata$province)
provinces %>% 
  map_df(~ plotdata %>% 
          mutate(highlight = province == .x,
                 panel = .x)) %>% 
  filter(!(province %in% c('New York State', 'Washington State')),
         !(panel %in% c('New York State', 'Washington State'))) %>% 
  ggplot(aes(x = days_since_10, y = diff, color = highlight, group = province, alpha = highlight, size = highlight)) +
  geom_line() +
  facet_wrap(~panel) +
  scale_alpha_manual(values = c(0.5, 1)) +
  scale_size_manual(values = c(1, 2)) +
  scale_color_manual(values = c('grey', '#E87D72')) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  labs(y = 'Growth Rate', 
       x = 'Days since 10th confirmed case',
       caption = glue::glue("Data from https://github.com/CSSEGISandData/COVID-19, updated {update_date}"),
       title = 'Daily growth rate by province since 10th confirmed case.') +
  scale_y_continuous(labels = scales::percent)
```


### Confirmed Cases Over Time

This chart shows the number of confirmed cases on each date.

```{r confirmed_by_time, echo=FALSE, warning=FALSE}
canada_plot(x = date)
```

## Deaths

### Deaths Since 3rd Death

This chart shows the number of deaths since the third date in each province or state.

```{r deaths_since_3rd, echo=FALSE, warning=FALSE}
canada_plot(deaths, filter = 3)
```

### Deaths Over Time

This shows the number of deaths on each date (min 3 deaths). Note, I am not sure why Quebec dips for a few days, but this is consistent with the data provided by the JHU dashboard.

```{r deaths_by_time, echo=FALSE, warning=FALSE}
canada_plot(deaths, date, filter = 3)
```

## "The Curve"

This plot shows the number of active cases (cumulative confirmed cases minus deaths and recoveries) over time since the date of the tenth confirmed case in each location. This is "the curve" that you've heard about trying to flatten. Note that a different y-axis is drawn for each location.

```{r the_curve, echo=FALSE, warning=FALSE}
canada_provinces %>% 
  group_by(province, date) %>% 
  tidyr::replace_na(list(deaths=0, recovered=0)) %>% 
  summarise(active = sum(confirmed) - sum(deaths) - sum(recovered),
            confirmed = sum(confirmed)) %>% 
  filter(confirmed >= 10) %>% 
  filter(n() > 5) %>% 
  mutate(days_since_10 = as.numeric(date - min(date))) %>% 
  mutate(active = zoo::rollmean(active, 5, NA)) %>% 
  ggplot(aes(x = days_since_10, y = active)) +
  geom_line(color = '#E87D72') +
  facet_wrap(~province, scales = 'free_y') +
  scale_y_continuous(labels = scales::comma) +
  labs(y = 'Active Cases', x = 'Days Since 10th Confirmed Case', title = '"The Curve" for selected Canadian Provinces and US States', caption = glue::glue("Data from https://github.com/CSSEGISandData/COVID-19, updated {maxdate}", maxdate = max(canada_provinces$date)))

```

## Country Totals

This chart shows the number of confirmed cases since the tenth confirmed cases in a few selected countries.

```{r countries, echo=FALSE, warning=FALSE}
filter <- 10
dat <-load_all_raw_data2() %>% 
  filter(country_region %in% c('US', 'United States', 'Canada', 'United Kingdom', 'UK', 'China', 'Mainland China', 'Italy')) %>% 
  mutate(country_region = case_when(
    country_region == 'UK' ~ 'United Kingdom',
    country_region == 'Mainland China' ~ 'China',
    TRUE ~ country_region
  )
  ) %>% 
  group_by(country_region, date) %>% 
  summarise(confirmed = sum(confirmed)) %>% 
  filter(confirmed > filter) %>% 
  mutate(first = as.numeric(date - min(date)))

label_data <- dat %>%
    filter(first == max(first))

M <- max(dat$first) + 5
dat %>% 
  mutate(
      day2 = filter * 2^(first / 2),
      day3 = filter * 2^(first / 3),
      day4 = filter * 2^(first / 4)
    ) %>% 
  ggplot(aes(x = first, y = confirmed, color = country_region)) +
  geom_point() +
  geom_line() +
  scale_y_log10(label = scales::comma) +
    geom_line(aes(y = day3, color = NULL), linetype = "dashed", color = "grey40") +
    geom_line(aes(y = day4, color = NULL), linetype = "dashed", color = "grey40") +
    annotate("text", x = M - 10, y = filter * 2^((M - 5) / 3:4 - .25), label = paste(3:4, "days"), color = "grey", hjust = 0) +
  ggthemes::theme_few() +
  ggrepel::geom_text_repel(data = label_data, aes(label = country_region)) +
  theme(legend.position = "none", panel.grid = element_line("grey95")) +
  labs(y = "Confirmed Cases", x = "Days Since 10th Confirmed Case",
       title = "Confirmed Cases by Country", caption = glue::glue("Data from https://github.com/CSSEGISandData/COVID-19, updated {maxdate}", maxdate = max(dat$date)))
```

## Data

The data that I used to produce these charts is below.

```{r download_data, echo=FALSE, warning=FALSE}
dt <- canada_provinces %>%
  group_by(country = country_region, province, date) %>%
  summarise(deaths = sum(deaths), confirmed = sum(confirmed)) %>%
  mutate(days_since_10 = date - as_date(min(ifelse(confirmed >= 10, date, NA), na.rm = T))) %>%
  ungroup() %>%
  transmute(date, country, province,
    days_since_10_cases = ifelse(days_since_10 >= 0, days_since_10, NA),
    confirmed, deaths
  ) %>%
  arrange(country, province, date) %>%
  DT::datatable(
    extensions = "Buttons", options = list(
      dom = "Bfrtip",
      buttons =
        list("copy", list(
          extend = "collection",
          buttons = c("csv", "excel"),
          text = "Download"
        ))
    )
  )

widgetframe::frameWidget(dt)
```


## References

* Dong E, Du H, Gardner L. An interactive web-based dashboard to track COVID-19 in real time. Lancet Infect Dis; published online Feb 19. https://doi.org/10.1016/S1473-3099(20)30120-1.